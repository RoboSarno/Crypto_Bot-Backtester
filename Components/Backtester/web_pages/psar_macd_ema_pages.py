...
def strat1_anal_page(risk, reward):
    """
    _summary_
        Home and explanation page for PSAR MAD EMA Strategy.
    Args:
    ...
    # make option menu for anal page
    selected = option_menu(None, ['SUMMARY- PSAR | MACD | EMA', 'Price Over Time', "Wallet Over Time"], 
    ...
    st.markdown("""---""")
    # home page
    if selected == 'SUMMARY- PSAR | MACD | EMA':
        st.write('''
                ## PSAR MACD EMA 30 min: (Please Read)
                
                #### PSAR Background:
                > Developed by J. Wells Wilder, the parabolic SAR indicator(**PSAR**), is used by traders to determine trend direction and potential reversals in price. The indicator uses a trailing stop and reverse method called "SAR," or stop and reverse, to identify suitable exit and entry points. 
                > 
                ...
                > **FIND OUT MORE** - https://www.investopedia.com/terms/p/parabolicindicator.asp
                
                
                #### MACD Background:
                > Moving average convergence divergence (MACD) is a momentum indicator that shows the relationship between two moving averages of a tickers price. 
                The MACD is calculated by subtracting the 26-period exponential moving average (EMA) from the 12-period EMA to find the covergence and divergence of a trend.
                
                > Signals are found whe the EMA's signals cross above (**buy**) or below (**sell**).  This cross over indicates if the market is being overbought or oversold.
                
                > MACD helps investors understand whether the bullish or bearish movement in the price is strengthening or weakening.
               
                > **FIND OUT MORE** - https://www.investopedia.com/terms/m/macd.asp  
                ...
                ''')
        temp = st.button('Update Strategy Table')
        # update strategy psar macd ema database info
        if temp:
            for t in ticker_label_list:
                current_tick = tick.Ticker(t, 1, '30m')
...
def strat1_anal_WOT(risk, reward):
    """
    _summary_
        Function to set up Wallet over Time page for Strat PSAR MACD EMA
    Args:
        risk (float): The amount of current shares im willing to risk  
        reward (float): The amount of current shares im willing to sell
    """
    ...
    connection = tick.Ticker(strat=1, interval='30m')
        ...
        for t in tickers:
            ...
            if len(wallet) <= 2:
                with st.expander(f'{t[:-3]} Wallet Info'):
                    ...
                    fig = psar_macd_ema_strat_graph(strat_df, wallet_buy, wallet_sell, sig_exists=False)
            ...
            if len(wallet) > 2:
                ...
                with st.expander(f'{t[:-3]} Wallet Info'):
                    ...
                    # display strat graph
                    fig = psar_macd_ema_strat_graph(strat_df, wallet_buy, wallet_sell, sig_exists=True)
                    plot = st.plotly_chart(fig, use_container_width=True, height=800) 
                    # display strat graph ------------------------------------------
            del strat_df, wallet_buy, wallet_sell
                    
def psar_macd_ema_strat_graph(strat_df, buy_df, sell_df, sig_exists=True):
    """_summary_
    ...
    """
    fig = make_subplots(rows=2, cols=1, subplot_titles=("PSAR MACD EMA Strategy", ''), vertical_spacing=0.1, shared_xaxes=True)
    ...
    fig.append_trace(
        go.Scatter(
            x=strat_df['datetime'], y=strat_df['ema_200'], opacity=0.5, 
            line={'width':3, 'color':'yellow'}, legendgroup='group1', name='EMA 200'
        ),  row=1, col=1)
    fig.append_trace(
        go.Scatter(
            x=strat_df['datetime'], y=strat_df['psarl'], opacity=0.3, 
            line={'width':3, 'color':'green'}, legendgroup='group1', name='PSAR Long'
        ),  row=1, col=1)
    fig.append_trace(
        go.Scatter(
            x=strat_df['datetime'], y=strat_df['psars'], opacity=0.3, 
            line={'width':3, 'color':'red'}, legendgroup='group1', name='PSAR Short'
        ),  row=1, col=1)
    fig.append_trace(
        go.Scatter(
            x=strat_df['datetime'], y=strat_df['macdh'], opacity=0.6, 
            line={'width':3, 'color':'blue'}, legendgroup='group2', name='MACD'
        ),  row=2, col=1)
    if sig_exists:
